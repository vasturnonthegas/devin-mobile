...
public enum PRReviewStatus: String, Codable, Sendable, CaseIterable {
    ...
    /// A terminal status will never change; polling stops here.
...
public struct PRReview: Codable, Hashable, Sendable {
    ...
    /// Host-prefixed, e.g. `github.com/owner/repo`.
    ...
    public init(status: PRReviewStatus, repoPath: String, prNumber: Int, commitSHA: String, createdAt: Date) {
        self.status = status
    ...
    /// True once the review reached a terminal status. Unknown statuses are treated as still in progress.
...
struct PRReviewCreateRequest: Encodable {
    let prURL: URL
