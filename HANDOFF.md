...
- [ ] **Widget** (WidgetKit): "Needs you: N" + top 3 sessions from `SessionSnapshot.load()`;
      tap → `SessionSnapshot.Entry.deepLink.url`.
...
- New Swift files under `DevinMobile/` are picked up automatically by XcodeGen (`sources: DevinMobile`);
  ...
  need new `targets:` entries plus an `entitlements:` block listing `group.ai.devin.mobile`). The
  app's `Info.plist` and `.entitlements` are generated by XcodeGen from `project.yml`; don't commit them.
